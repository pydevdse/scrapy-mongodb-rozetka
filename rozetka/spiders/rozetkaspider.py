...
class RozetkaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        pages = response.xpath(
            '//a[@class="pagination__link ng-star-inserted"]//text()'
        )
    ...
    def parse_product(self, response):
        product_json = json.loads(
            response.xpath('.//script[@data-seo="Product"]/text()').extract_first()
        )
        meta = response.meta
        meta["product"] = product_json
        yield scrapy.Request(
            response.url + "characteristics/",
            meta=meta,
    ...
    def parse_characteristics(self, response):
        ...
        for c in response.xpath(
            '//section/dl[@class="characteristics-full__list"]/div[@class="characteristics-full__item ng-star-inserted"]'
        ):
            parametr_label = c.xpath(
                './/dt[@class="characteristics-full__label"]/span//text()'
            ).extract_first()
            parametr_value = c.xpath(
                './/dd[@class="characteristics-full__value"]/ul/li//text()'
            ).extract_first()
